...
export default function PrivacyPage() {
  ...
  return <LegalPage eyebrow="Privacy · Draft for legal review" title="Small data footprint. Clear public data." intro="StateBid has no bidder accounts and does not collect a bidder email inside the application. Stripe may collect billing and receipt details within hosted Checkout.">
    ...
    <h2>5. Security and operational data</h2><p>We process short-lived IP and browser signals for rate limits, abuse prevention, Turnstile checks, webhook logs, error monitoring, moderation records, and operator-authentication audit. Public reports can be submitted without an account or email.</p>
    <h2>6. Retention</h2><ul><li>Temporary listing previews and unused uploads expire after one hour and are removed through automated cleanup.</li><li>Abandoned bid attempts are retained for up to 30 days for reconciliation and then removed or minimized.</li><li>Raw deduplicated click events are retained for {rawClickDays} days; non-identifying daily aggregates may be kept longer for public statistics.</li><li>Security and ordinary error logs are retained for up to 90 days unless an incident requires longer investigation.</li><li>Verified payment, reversal, moderation, and accounting records are retained for the service lifetime and any longer period required by applicable law.</li></ul>
    <h2>7. Processors and transfers</h2><p>Cloudflare provides hosting, D1 database, R2 object storage, security, and delivery. Stripe provides payment processing, fraud tools, receipts, and optional tax services. These providers may process data internationally using their contractual transfer mechanisms. Exact entities and regional settings must be documented before live launch.</p>
    <h2>8. Legal bases</h2><p>We process transaction and placement data to perform the agreement; fraud, security, measurement, and moderation data for legitimate interests; records to meet legal obligations; and optional technologies on consent where applicable.</p>
    <h2>9. Your rights</h2><p>Depending on the GDPR and local law, you may request access, correction, erasure, restriction, portability, or object to processing, and may complain to the Swedish Authority for Privacy Protection (IMY). Public listing immutability does not override mandatory privacy rights; requests are assessed against legal and transactional record obligations.</p>
    <h2>10. No sale of personal data</h2><p>StateBid does not sell bidder or visitor personal data and does not use application data to build cross-site advertising profiles.</p>
  </LegalPage>;
}
