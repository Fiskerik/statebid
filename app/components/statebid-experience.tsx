...
export function StateBidExperience({ initialSnapshot }: { initialSnapshot: BoardSnapshot }) {
  ...
  return <main className="site-shell">
    ...
    <section className="market-grid" id="map">
      ...
      <div className="map-column"><div className="map-toolbar"><div><span className="map-pulse" /> Live ownership map</div><span>Updated {relativeTime(snapshot.generatedAt)}</span></div>
        <div className="map-card"><div className="map-grid-bg" aria-hidden="true" /><MapSurface selected={selected} positions={positions} onSelect={setSelected} />
          {detailsVisible ? <div className="state-detail-card" role="status" aria-live="polite"><div className="detail-topline"><span>{state.name}</span><span>{position ? 'Claimed' : 'Available'}</span><button className="detail-hide" type="button" onClick={() => setDetailsVisible(false)} aria-label="Hide state details"><X size={13} /></button></div>
            {position ? <><a className="detail-owner detail-owner-link" href={`/go/${selected}`} target="_blank" rel="sponsored nofollow noopener" aria-label={`Visit ${position.listing.title}`}><Logo listing={position.listing} /><div><strong>{position.listing.title}</strong><small>{position.listing.normalizedKey.replace(/^\w+:/, '')}</small></div><span>{formatMoney(position.totalCents)} <ExternalLink size={13} /></span></a><div className="detail-metrics"><span><small>Last 24h</small>{formatMoney(position.dailyCents)}</span><span><small>Clicks</small>{new Intl.NumberFormat('en-US').format(position.clicks)}</span></div><div className="detail-links"><a href={`/go/${selected}`} target="_blank" rel="sponsored nofollow noopener">Visit sponsor <ExternalLink size={13} /></a><a href={`/state/${selected.toLowerCase()}`}>Share page</a></div><button className="claim-button" type="button" onClick={openClaim}>Claim {state.code} for {formatMoney(position.takeoverCents)} <ArrowUpRight size={16} /></button></>
          ...
          </div> : <button className="detail-show" type="button" onClick={() => setDetailsVisible(true)}>Show {state.name} details <ArrowUpRight size={13} /></button>}
    ...
    <section className="all-states-section" id="all-states"><div><span className="eyebrow"><span /> Accessible list</span><h2>All 50 states</h2></div><div className="state-chip-grid">{[...STATE_BY_CODE.values()].map((item) => { const leader = positions.get(item.code); const bidders = snapshot.topBidders[item.code] ?? []; const expanded = expandedState === item.code; return <div key={item.code} className={`state-chip ${expanded ? 'is-expanded' : ''}`}><button className="state-chip-toggle" type="button" aria-expanded={expanded} onClick={() => { setSelected(item.code); setExpandedState(expanded ? null : item.code); }}><span className="state-chip-summary"><span style={leader ? { borderColor: leader.stateBorderColor, color: leader.stateBorderColor } : undefined}>{item.code}</span><strong>{item.name}</strong>{leader ? <span className="state-chip-leader"><Logo listing={leader.listing} small /><strong>{leader.listing.title}</strong></span> : <small>$1 worth · claim</small>}<small>{leader ? `${formatMoney(leader.totalCents)} worth` : ''}</small></span></button>{expanded ? <div className="state-bidders-dropdown" role="region" aria-label={`Top bidders for ${item.name}`}><span className="state-bidders-title">Top 3 bidders</span>{bidders.length ? bidders.slice(0, 3).map((bidder, index) => <a className="state-bidder-row" key={bidder.listing.id} href={bidder.listing.canonicalUrl} target="_blank" rel="sponsored nofollow noopener" aria-label={`Visit ${bidder.listing.title}`}><span className="rank">{String(index + 1).padStart(2, '0')}</span><Logo listing={bidder.listing} small /><span><strong>{bidder.listing.title}</strong><small>{formatMoney(bidder.totalCents)}</small></span></a>) : <span className="state-bidders-empty">No verified bidders yet.</span>}</div> : null}</div>; })}</div></section>
