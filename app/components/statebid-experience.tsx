...
export function StateBidExperience({ initialSnapshot }: { initialSnapshot: BoardSnapshot }) {
  ...
  return <main className="site-shell">
    ...
    <section className="all-states-section" id="all-states"><div><span className="eyebrow"><span /> Accessible list</span><h2>All 50 states</h2></div><div className="state-chip-grid">{[...STATE_BY_CODE.values()].map((item) => { const leader = positions.get(item.code); const bidders = snapshot.topBidders[item.code] ?? []; const expanded = expandedState === item.code; return <button key={item.code} className={`state-chip ${expanded ? 'is-expanded' : ''}`} aria-expanded={expanded} onClick={() => { setSelected(item.code); setExpandedState(expanded ? null : item.code); }}><span className="state-chip-summary"><span style={leader ? { borderColor: leader.stateBorderColor, color: leader.stateBorderColor } : undefined}>{item.code}</span><strong>{item.name}</strong><small>{leader ? `${formatMoney(leader.totalCents)} worth` : '$1 worth · claim'}</small></span>{expanded ? <span className="state-bidders-dropdown" role="region" aria-label={`Top bidders for ${item.name}`}><span className="state-bidders-title">Top 3 bidders</span>{bidders.length ? bidders.slice(0, 3).map((bidder, index) => <span className="state-bidder-row" key={bidder.listing.id}><span className="rank">{String(index + 1).padStart(2, '0')}</span><Logo listing={bidder.listing} small /><span><strong>{bidder.listing.title}</strong><small>{formatMoney(bidder.totalCents)}</small></span></span>) : <span className="state-bidders-empty">No verified bidders yet.</span>}</span> : null}</button>; })}</div></section>
